...
export const Personnel: React.FC = () => {
  ...
  const loadData = async () => {
    const data = await db.getSoldiers();
    setSoldiers(data);
  };
  ...
  const handleSave = async () => {
    ...
    const result = await db.saveSoldier(newSoldier);

    if (result.error) {
      alert(`Erro ao salvar militar: ${result.error.message || 'Erro desconhecido'}`);
      console.error('[Personnel] Falha no salvamento:', result.error);
    } else {
      alert("✅ Militar salvo com sucesso no Supabase!");
  ...
  const handleImport = async () => {
    ...
    for (const line of lines) {
      ...
      await db.saveSoldier(newSoldier);
      count++;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="relative w-full md:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-pm-500 outline-none dark:bg-slate-800 dark:border-slate-700 dark:text-white"
            value={searchTerm}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-x-auto border border-gray-200 dark:border-slate-700">
        <table className="w-full text-left min-w-[800px]">
          <thead className="bg-gray-50 dark:bg-slate-700 border-b border-gray-200 dark:border-slate-600">
            <tr>
              <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Posto/Grad</th>
              <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Quadro</th>
              <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Nome de Guerra</th>
              <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Telefone</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
            {filteredSoldiers.map(s => (
              <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300 font-medium">{s.rank}</td>
                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 font-medium text-[10px] uppercase">{s.cadre || '-'}</td>
                ...
                <td className="px-6 py-4 text-sm">
                  <div className="flex flex-col">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 w-fit mb-1">
                      {s.role} <span className="ml-1 opacity-75">{s.roleShort}</span>
                    </span>
                    {s.team && <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Equipe: {s.team}</span>}
                  </div>
                </td>
                <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{s.sector}</td>
                <td className="px-6 py-4 text-sm">
                  <div className="flex flex-col items-start space-y-1">
                    <span className={`inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full text-xs font-medium 
                      ${s.status === Status.ATIVO ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                        s.status === Status.FOLGA ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}>
                    ...
                    {s.status !== Status.ATIVO && s.status !== Status.FOLGA && s.absenceStartDate && s.absenceEndDate && (
                      <span className="text-[10px] text-gray-500 dark:text-gray-400 font-mono pl-1">
            ...
            {filteredSoldiers.length === 0 && (
              <tr>
                <td colSpan={8} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">Nenhum militar encontrado.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 w-full max-w-2xl overflow-y-auto max-h-[95vh] animate-in fade-in zoom-in-95 duration-200 border border-gray-100 dark:border-slate-700">
            <div className="flex justify-between items-center border-b dark:border-slate-700 pb-4 mb-4">
              <h3 className="text-xl font-bold text-gray-800 dark:text-white">{editingId ? 'Editar Dados do Militar' : 'Cadastrar Novo Militar'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"><X size={24} /></button>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-slate-700/50 p-4 rounded-lg border border-gray-100 dark:border-slate-600">
                <h4 className="text-sm font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wide mb-3 flex items-center">
                  <UserCheck size={16} className="mr-2" /> Dados Pessoais
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="col-span-2 md:col-span-1">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nome de Guerra</label>
                    <input type="text" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 focus:ring-pm-500 focus:border-pm-500 font-bold dark:bg-slate-800 dark:text-white"
                      placeholder="Ex: SILVA"
                      value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                  </div>
                  <div className="col-span-2 md:col-span-1">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Telefone / WhatsApp</label>
                    <input type="text" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
                      placeholder="(85) 90000-0000"
                      value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nome Completo (Completo)</label>
                    <input type="text" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Posto / Graduação</label>
                    <select className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Quadro</label>
                    <select className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Numeral</label>
                    <input type="text" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
                      placeholder="Somente números"
                      value={formData.matricula} onChange={e => setFormData({ ...formData, matricula: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Matrícula Funcional (M.F.)</label>
                    <input type="text" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
                      placeholder="Ex: 123.456-1-X"
                      value={formData.mf} onChange={e => setFormData({ ...formData, mf: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Função Principal</label>
                    <select className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Setor de Atuação</label>
                    <input type="text" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white"
                      placeholder="Ex: Ambulância, Administrativo..."
                      value={formData.sector} onChange={e => setFormData({ ...formData, sector: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Legenda Curta (Impressão)</label>
                    <input type="text" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 dark:bg-slate-800 dark:text-white" placeholder="Ex: (M), (F), (1)"
                      value={formData.roleShort} onChange={e => setFormData({ ...formData, roleShort: e.target.value })} />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-sm font-black text-blue-900 dark:text-blue-300 uppercase">Equipe / Plantão</label>
                    <select className="mt-1 block w-full border-2 border-blue-200 dark:border-blue-700 rounded-md p-2 font-bold dark:bg-slate-800 dark:text-white"
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1">Vincula o militar ao simulador de escalas.</p>
                  </div>
                </div>
              </div>

              {/* GROUP 3: Status */}
              <div className={`p-4 rounded-lg border ${formData.status === Status.ATIVO ? 'bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-800'}`}>
                <h4 className={`text-sm font-bold uppercase tracking-wide mb-3 flex items-center ${formData.status === Status.ATIVO ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'}`}>
                ...
                <div className="mb-4">
                  ...
                  <select className="block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 font-bold dark:bg-slate-800 dark:text-white"
                ...
                {formData.status !== Status.ATIVO && formData.status !== Status.FOLGA && (
                  <div className="grid grid-cols-2 gap-4 animate-in fade-in">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Início do Afastamento</label>
                      <input type="date" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded p-2 bg-white dark:bg-slate-800 dark:text-white"
                        value={formData.absenceStartDate || ''}
                        onChange={e => setFormData({ ...formData, absenceStartDate: e.target.value })} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Fim do Afastamento</label>
                      <input type="date" className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded p-2 bg-white dark:bg-slate-800 dark:text-white"
                ...
                {formData.status === Status.FOLGA && (
                  <div className="animate-in fade-in">
                    <label className="block text-xs font-bold text-yellow-800 dark:text-yellow-300 uppercase mb-1">Motivo da Folga / Referência</label>
                    <input
                      type="text"
                      placeholder="Ex: Serviço Extra, Compensação, Doação de Sangue..."
                      className="w-full border border-yellow-300 dark:border-yellow-700 rounded p-2 focus:ring-yellow-500 focus:border-yellow-500 bg-white dark:bg-slate-800 dark:text-white"
              ...
              <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg border border-orange-100 dark:border-orange-800">
                <h4 className="text-sm font-bold text-orange-800 dark:text-orange-300 uppercase tracking-wide mb-3 flex items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="availableExtra" className="text-sm font-medium text-gray-700 dark:text-gray-300">Disponível para Rodízio Extra?</label>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Ordem na Fila (Sequência)</label>
                    <input
                      type="number"
                      className="mt-1 block w-full border border-gray-300 dark:border-slate-600 rounded-md p-2 text-sm dark:bg-slate-800 dark:text-white"
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 w-full max-w-2xl border border-gray-100 dark:border-slate-700">
            <h3 className="text-lg font-bold mb-2 flex items-center dark:text-white">
            ...
            <textarea
              className="w-full h-64 p-4 border rounded-lg font-mono text-sm bg-gray-50 dark:bg-slate-900 focus:ring-2 focus:ring-pm-500 outline-none dark:text-white dark:border-slate-600"
              placeholder={`Exemplo:
...
Sgt PM	Maria Souza	Enfermeiro	Ambulância	987654	8888-7777`}
            ...
            <div className="mt-6 flex justify-end space-x-3">
              <button onClick={() => setIsImportModalOpen(false)} className="px-4 py-2 border rounded-md hover:bg-gray-50 dark:border-slate-600 dark:hover:bg-slate-700 dark:text-white">Cancelar</button>
