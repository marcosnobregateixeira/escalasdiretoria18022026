...
class StoreService {
  ...
  getAdminPassword(): string {
    return localStorage.getItem('admin_password') || '123';
  }
  ...
  login(role: 'ADMIN' | 'USER'): User | null {
    const user: User = { username: role === 'ADMIN' ? 'Administrador' : 'Visitante', role };
  ...
  async saveSoldier(soldier: Soldier): Promise<{ data: any; error: any }> {
    try {
      console.log('[StoreService] Salvando no Supabase:', soldier.name);
      const payload = {
        id: soldier.id,
        name: soldier.name,
      ...
      if (error) {
        console.error('[StoreService] Erro Supabase:', error);
      ...
      console.error('[StoreService] Exceção:', e);
