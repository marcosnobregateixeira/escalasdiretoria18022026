...
import { Roster, RosterSection, Soldier, Rank, Status, Shift, BankTransaction, AppSettings } from '../types';
...
export const RosterManager: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const allRosters = await db.getRosters();
      allRosters.sort((a, b) => new Date(b.creationDate || 0).getTime() - new Date(a.creationDate || 0).getTime());
      setRosters(allRosters);

      const solds = await db.getSoldiers();
      setSoldiers(solds);

      if (!activeTab && sett.rosterCategories.length > 0) {
        setActiveTab(sett.rosterCategories[0].id);
      }
    } catch (error) {
  ...
  const activeCategory = useMemo(() => {
    ...
    return settings.rosterCategories.find(c => c.id === activeTab) || settings.rosterCategories[0];
  }, [settings, activeTab]);
  ...
  const getCycleIndexForDate = (dateObj: Date) => {
    if (!settings || !settings.shiftCycleRefDate) return 0;
  ...
  const handleAutoGenerate = async () => {
    ...
    await updateRoster({ ...selectedRoster, shifts: finalShifts });
  ...
  const handleCreateRoster = async () => {
    if (!settings || !newRosterMeta.startDate || !newRosterMeta.endDate) return alert("Selecione o período da escala.");
  ...
  const handleDeleteRoster = async (targetId?: string) => {
    ...
    if (confirm(`ATENÇÃO: Deseja excluir PERMANENTEMENTE a escala "${rosterToDelete?.title || 'Selecionada'}"?`)) {
      await db.deleteRoster(idToDelete);
  ...
  const addRow = async (sIdx: number) => {
    ...
    await updateRoster({ ...selectedRoster, sections: newSections });
  };

  const deleteRow = async (sIdx: number, rIdx: number) => {
    if (!selectedRoster || !selectedRoster.sections) return;
  ...
  const deleteSection = async (sIdx: number) => {
    ...
    await updateRoster({ ...selectedRoster, sections: selectedRoster.sections.filter((_, idx) => idx !== sIdx) });
  };

  const updateSectionTitle = async (sIdx: number, title: string) => {
    if (!selectedRoster || !selectedRoster.sections) return;
  ...
  const updateRowLabel = async (sIdx: number, rIdx: number, label: string) => {
    if (!selectedRoster || !selectedRoster.sections) return;
    const newSections = selectedRoster.sections.map((sec, idx) => {
  ...
  const updateShift = async (date: string, period: string, soldierId: string) => {
    if (!selectedRoster) return;
    ...
    await updateRoster({ ...selectedRoster, shifts: newShifts });
    setIsSearchOpen(false);
  };

  const addShiftToCell = async (date: string, period: string, soldierId: string) => {
    if (!selectedRoster) return;
  ...
  const removeShiftFromCell = async (shift: any) => {
    if (!selectedRoster) return;
    const newShifts = selectedRoster.shifts.filter(s => s !== shift);
    await updateRoster({ ...selectedRoster, shifts: newShifts });
  };

  const updateShiftNote = async (shiftToUpdate: Shift, newNote: string) => {
    if (!selectedRoster) return;
  ...
  const handleAddSoldierToExtra = async (soldierId: string) => {
    if (!selectedRoster) return;
    ...
    if (soldier) {
      ...
      await db.saveSoldier(updatedSoldier);
  ...
  const handleRemoveSoldierFromExtra = async (soldierId: string) => {
    if (!selectedRoster) return;
  ...
  const handleAddColumn = async () => {
    ...
    await updateRoster({ ...selectedRoster, customHeaders: newHeaders });
  };
  ...
  const updateCustomHeader = async (index: number, value: string) => {
    if (!selectedRoster) return;
    const newHeaders = [...(selectedRoster.customHeaders || [])];
  ...
  const updateCustomCell = async (soldierId: string, colIndex: number, value: string) => {
    if (!selectedRoster) return;
    const newShifts = selectedRoster.shifts.map(s => {
  ...
  const updateRoster = async (updated: Roster) => {
    setSelectedRoster(updated);
    await db.saveRoster(updated);
    setRosters(prev => prev.map(r => r.id === updated.id ? updated : r));
  };

  const updateSituationText = async () => {
    if (!selectedRoster) return;
    ...
    await updateRoster({ ...selectedRoster, situationText: newText });
  };
    ...
    <div className="h-full flex flex-col space-y-4">
      ...
      <div className="relative z-30">
        <button
          ...
          className="w-full bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-all group"
        >
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-pm-100 dark:bg-slate-700 rounded-lg text-pm-800 dark:text-pm-200">
              <DynamicIcon name={activeCategory?.icon || 'Scale'} size={24} />
            </div>
            <div className="text-left">
              <p className="text-[10px] uppercase font-bold text-gray-400 dark:text-gray-500 tracking-wider">Categoria da Escala Selecionada</p>
              <h2 className="text-xl font-black text-pm-900 dark:text-white uppercase tracking-tight">{activeCategory?.name || 'Selecione...'}</h2>
            </div>
          </div>
        ...
        {isCategoryMenuOpen && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-gray-100 dark:border-slate-700 overflow-hidden animate-in fade-in slide-in-from-top-2 z-40">
            <div className="max-h-[300px] overflow-y-auto divide-y divide-gray-100 dark:divide-slate-700">
              {settings.rosterCategories.map(cat => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 hover:bg-pm-50 dark:hover:bg-slate-700 transition-colors
                     ${activeTab === cat.id ? 'bg-pm-50 dark:bg-slate-700 border-l-4 border-pm-700' : 'border-l-4 border-transparent'}`}
                >
                  <div className="flex items-center space-x-3">
                    <DynamicIcon name={cat.icon} className={activeTab === cat.id ? 'text-pm-700 dark:text-white' : 'text-gray-400'} size={20} />
                    <span className={`uppercase text-sm font-bold ${activeTab === cat.id ? 'text-pm-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}>{cat.name}</span>
                  </div>
                  {activeTab === cat.id && <Check size={16} className="text-pm-700 dark:text-white" />}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-3 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
        <div className="flex items-center space-x-3">
          <Scale size={18} className="text-pm-900 dark:text-white" />
          <select
            className="border-gray-200 dark:border-slate-600 rounded-lg text-xs font-black text-pm-900 dark:text-white focus:ring-pm-500 bg-gray-50 dark:bg-slate-700 px-3 py-2 min-w-[280px]"
        ...
        {selectedRoster && (
          <div className="flex space-x-2">
            {isAdmin && (
              <button onClick={() => handleDeleteRoster()} className="bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-300 hover:bg-red-600 hover:text-white px-4 py-2 rounded-lg flex items-center space-x-2 font-black text-[10px] shadow-sm transition-all border border-red-200 dark:border-red-800">
      ...
      {isCreating ? (
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl max-w-lg mx-auto border border-gray-100 dark:border-slate-700 animate-in zoom-in-95">
          <h3 className="text-xl font-black text-pm-900 dark:text-white uppercase mb-6 border-b dark:border-slate-700 pb-4">Configurar Nova Escala</h3>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" className="w-full p-3 border dark:border-slate-600 rounded-lg font-bold dark:bg-slate-700 dark:text-white" value={newRosterMeta.startDate} onChange={e => setNewRosterMeta({ ...newRosterMeta, startDate: e.target.value })} />
              </div>
              <div>
                ...
                <input type="date" className="w-full p-3 border dark:border-slate-600 rounded-lg font-bold dark:bg-slate-700 dark:text-white" value={newRosterMeta.endDate} onChange={e => setNewRosterMeta({ ...newRosterMeta, endDate: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Data de Criação (Emissão)</label>
              <input type="date" className="w-full p-3 border dark:border-slate-600 rounded-lg font-bold dark:bg-slate-700 dark:text-white" value={newRosterMeta.creationDate} onChange={e => setNewRosterMeta({ ...newRosterMeta, creationDate: e.target.value })} />
            </div>
            <div>
              <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Título da Planilha</label>
              <input type="text" className="w-full p-3 border dark:border-slate-600 rounded-lg uppercase font-black dark:bg-slate-700 dark:text-white" placeholder="EX: CAMINHADA COM MARIA" value={newRosterMeta.title} onChange={e => setNewRosterMeta({ ...newRosterMeta, title: e.target.value })} />
            </div>

            {(activeTab !== 'cat_extra') && (
              <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-xl border border-purple-100 dark:border-purple-800 flex items-start space-x-3 cursor-pointer hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors" onClick={() => setAutoFillOnCreate(!autoFillOnCreate)}>
                <div className={`w-5 h-5 rounded border flex items-center justify-center mt-0.5 ${autoFillOnCreate ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white dark:bg-slate-700 border-gray-300 dark:border-slate-600'}`}>
                  {autoFillOnCreate && <Check size={14} />}
                </div>
                <div>
                  <p className="text-xs font-black text-purple-900 dark:text-purple-300 uppercase">Preencher Automaticamente</p>
                  <p className="text-[10px] text-purple-700 dark:text-purple-400 leading-tight mt-0.5">
                    {activeTab === 'cat_amb'
            ...
            <div className="flex justify-end space-x-3 pt-4">
              <button onClick={() => setIsCreating(false)} className="px-6 py-2 text-gray-400 font-bold hover:text-gray-600 dark:hover:text-gray-200 transition-colors">Cancelar</button>
        ...
        // MANTÉM OS CONTAINERS DE IMPRESSÃO (Folhas Brancas) para garantir que a impressão seja WYSIWYG
        selectedRoster.type === 'cat_extra' ? (
          <div className="flex-1 overflow-auto bg-gray-200/80 dark:bg-slate-900 flex justify-center p-8 rounded-lg border-inner shadow-inner">
            ...
            <div className="w-[210mm] min-h-[297mm] bg-white text-black shadow-2xl relative flex flex-col mx-auto" style={{ padding: '20mm', fontFamily: 'Arial, Helvetica, sans-serif' }}>
              ...
              <div className="mb-6 text-[12pt] text-left relative group text-black">
                ...
                <textarea
                  readOnly={!isAdmin}
                  className={`w-full uppercase outline-none border border-transparent rounded p-1 resize-none overflow-hidden bg-transparent text-black ${isAdmin ? 'hover:border-gray-300 focus:border-pm-500' : ''}`}
                  rows={2}
              ...
              <div className="text-right text-[12pt] mb-16 mt-auto relative group text-black">
                ...
                <input
                  readOnly={!isAdmin}
                  type="date"
                  className={`bg-transparent font-bold ml-1 outline-none text-right cursor-pointer text-black ${isAdmin ? '' : 'pointer-events-none'}`}
              ...
              <div className="text-center mb-4 break-inside-avoid text-black">
          ...
          <div className="flex-1 overflow-auto bg-gray-200/80 dark:bg-slate-900 flex justify-center p-8 rounded-lg border-inner shadow-inner">
            ...
            <div className="w-[297mm] min-h-[210mm] bg-white text-black shadow-2xl relative flex flex-col mx-auto" style={{ padding: '10mm', fontFamily: 'Arial, Helvetica, sans-serif' }}>
              ...
              <div className="mt-2 text-[8pt] relative text-black">
            ...
            <div className="w-[210mm] min-h-[297mm] bg-white text-black shadow-2xl relative flex flex-col mx-auto" style={{ padding: '8mm', fontFamily: 'Arial, Helvetica, sans-serif' }}>
              <header className="text-center mb-4 relative">
                ...
                <div className="mx-20">
                  ...
                  <input
                    readOnly={!isAdmin}
                    className={`w-full text-center text-[13pt] font-black uppercase mt-1 outline-none text-black ${isAdmin ? 'hover:bg-gray-50 focus:bg-yellow-50' : 'pointer-events-none'}`}
                    value={selectedRoster.title}
                    onChange={e => updateRoster({ ...selectedRoster, title: e.target.value.toUpperCase() })}
                  />
                  <div className="text-[9pt] font-bold mt-1 uppercase text-black">
              ...
              <div className="bg-[#cbd5b0] border-2 border-black border-b-0 p-1 text-center">
                <input
                  readOnly={!isAdmin}
                  className={`w-full bg-transparent text-center font-bold text-[8pt] uppercase outline-none text-black ${isAdmin ? 'placeholder-gray-500' : 'pointer-events-none'}`}
                      ...
                      <React.Fragment key={sIdx}>
                        <tr className="h-4 bg-[#cbd5b0]">
                          <td colSpan={dates.length} className="border border-black p-0 text-center font-bold text-[7pt] uppercase tracking-wide leading-none align-middle group relative">
                            ...
                            <input
                              readOnly={!isAdmin}
                              className={`w-full bg-transparent text-center font-bold text-[9pt] uppercase outline-none text-black ${isAdmin ? '' : 'pointer-events-none'}`}
                              value={sec.title}
              ...
              <div className="mt-1 flex flex-col justify-end text-black">
                ...
                <div className="flex flex-col items-center justify-center mt-2 relative">
                  <div className="absolute right-0 top-0 text-[8pt] font-bold text-black">
                    {settings.city},
                    <input
                      readOnly={!isAdmin}
                      type="date"
                  ...
                  <div className="text-center w-1/3 mx-auto mt-4 text-black">
                    <div className="w-full border-b border-black mb-0.5"></div>
                    <p className="font-bold uppercase text-[8pt] leading-none">{settings.directorName} – {settings.directorRank}</p>
                    <p className="uppercase text-[7pt] leading-none mt-1">{settings.directorRole}</p>
                    <p className="uppercase text-[7pt] leading-none mt-1">{settings.directorMatricula}</p>
        ...
        <div className="flex-1 overflow-auto bg-gray-50 dark:bg-slate-900 p-6 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-inner">
          <div className="flex items-center justify-center h-full">
            <div className="text-center opacity-50">
              <Scale size={64} className="mx-auto mb-4 text-gray-400 dark:text-gray-600" />
              <p className="font-bold text-gray-500 dark:text-gray-400 uppercase">Selecione ou crie uma escala para começar.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {isAutoModalOpen && (
        <div className="fixed inset-0 bg-pm-950/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md animate-in zoom-in-95 border border-purple-200 dark:border-purple-800">
            <div className="p-6 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-purple-50 dark:bg-purple-900/20 rounded-t-2xl">
              <h3 className="font-black text-purple-900 dark:text-purple-300 uppercase flex items-center text-lg">
                <Wand2 className="mr-2 text-purple-600 dark:text-purple-400" size={24} /> Gerar Escala Automática
              </h3>
              <button onClick={() => setIsAutoModalOpen(false)} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400"><CloseIcon size={24} /></button>
            </div>
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase ml-1">Data Inicial</label>
                  <input
                    type="date"
                    className="w-full p-3 bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl font-bold text-gray-800 dark:text-white outline-none focus:ring-2 focus:ring-purple-500"
                    value={autoDates.start}
                    onChange={e => setAutoDates({ ...autoDates, start: e.target.value })}
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase ml-1">Data Final</label>
                  <input
                    type="date"
                    className="w-full p-3 bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl font-bold text-gray-800 dark:text-white outline-none focus:ring-2 focus:ring-purple-500"
                    value={autoDates.end}
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 bg-pm-950/80 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          {/* ... modal de busca ... */}
          <div className="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden border border-pm-800 dark:border-slate-700 animate-in zoom-in-95">
            <div className="bg-pm-900 dark:bg-slate-950 p-6 text-white flex justify-between items-center">
              <div className="flex items-center space-x-3">
              ...
              <button onClick={() => setIsSearchOpen(false)} className="hover:bg-pm-800 dark:hover:bg-slate-800 p-2 rounded-full transition-colors"><CloseIcon size={24} /></button>
            </div>
            <div className="p-4 bg-pm-50 dark:bg-slate-900 border-b dark:border-slate-700">
              <div className="relative">
                <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
                <input autoFocus className="w-full p-4 pl-12 border dark:border-slate-600 rounded-xl font-bold shadow-md outline-none focus:ring-2 focus:ring-pm-500 dark:bg-slate-700 dark:text-white" placeholder="PESQUISAR NOME OU MATRÍCULA..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
              </div>
            </div>
            <div className="max-h-[50vh] overflow-y-auto divide-y divide-gray-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
              {selectedRoster?.type !== 'cat_extra' && selectedRoster?.type !== 'cat_adm' && activeSearchCell && (
                <button onClick={() => updateShift(activeSearchCell.date, activeSearchCell.period, '')} className="w-full p-5 text-red-600 font-black uppercase text-[11px] hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center justify-center space-x-2 transition-colors">
              ...
              {filteredSoldiersForSearch.length > 0 ? filteredSoldiersForSearch.map(s => (
                <button
                  ...
                  className="w-full text-left p-4 hover:bg-pm-50 dark:hover:bg-slate-700 flex items-center group transition-all"
                >
                  <div className="p-2 bg-gray-100 dark:bg-slate-600 rounded-full mr-3 text-gray-500 dark:text-gray-300">
                    <User size={20} />
                  </div>
                  <div className="flex-1">
                    <p className="font-bold text-sm text-pm-900 dark:text-white uppercase leading-none">{getAbbreviatedRank(s.rank)} {s.name}</p>
                    <div className="flex items-center space-x-2 mt-1">
                      <span className="text-[10px] font-mono text-gray-500 dark:text-gray-400">{s.matricula || 'S/N'}</span>
                      <span className="text-[10px] bg-pm-100 dark:bg-slate-600 text-pm-700 dark:text-gray-200 px-1.5 py-0.5 rounded font-bold uppercase">{s.roleShort}</span>
                    </div>
                  </div>
                  {selectedRoster?.type !== 'cat_extra' && (
                ...
                <div className="p-8 text-center text-gray-400 dark:text-gray-500 font-bold uppercase text-xs">
