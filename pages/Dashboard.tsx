// Build Vercel: 2026-02-18-22-55 🚀
import React, { useState, useMemo, useEffect } from 'react';
import { db } from '../services/store';
import { supabase } from '../services/supabase';
import { Users, AlertTriangle, Search, Clock, UserMinus, ShieldCheck, BookOpen, PlusCircle, MinusCircle, History, X, Save, Trash2, ArrowLeft, TrendingUp, TrendingDown, Wallet, FileText, Eye, Info, Wand2, BarChart3, Medal, ListOrdered, CheckCircle2, RotateCw, Briefcase, MapPin, User, Calendar, Plus } from 'lucide-react';
import { Rank, Status, Soldier, BankTransaction, Roster, Cadre, ExtraDutyHistory, AppSettings, Role } from '../types';
...
export const Dashboard: React.FC = () => {
   ...
   const testSupabase = async () => {
      ...
      console.log('--- INICIANDO TESTE SUPABASE ---');
      try {
         ...
         alert('✅ SUCESSO! O sistema conseguiu se conectar ao Supabase e ler as configurações.');
         console.log('Dados do Supabase:', data);
      } catch (e: any) {
         console.error('Erro no teste:', e);
         alert('❌ FALHA NO TESTE: ' + (e.message || 'Erro desconhecido. Verifique o console e as chaves no .env.local'));
      } finally {
         setIsTestingSupabase(false);
      }
   };

   const checkXimenes = async () => {
      // ... (mantido código anterior)
   };

   const testXixo = async () => {
      try {
         alert('🚀 Iniciando teste: Tentando cadastrar Xixo...');
         const xixo: Soldier = {
            id: 'xixo-' + Date.now(),
            name: 'Xixo',
            fullName: 'Policial Xixo (Teste)',
            rank: Rank.SD,
            role: Role.MOTORISTA,
            roleShort: '(M)',
            sector: 'Geral',
         ...
         console.log('Enviando Xixo para db.saveSoldier...', xixo);
         await db.saveSoldier(xixo);

         alert('✅ O comando db.saveSoldier terminou! Verificando agora no banco...');
         ...
         if (error) {
            alert('❌ Erro após salvar: Salvei mas não consegui encontrar na busca imediata: ' + error.message);
         } else {
            alert('🏆 SUCESSO TOTAL! Xixo foi salvo e confirmado no Supabase.\nID: ' + data.id);
         }
      } catch (e: any) {
         alert('💥 CRASH NO TESTE: ' + e.message);
      }
   };
   ...
   useEffect(() => {
      const loadData = async () => {
         setIsLoading(true);
         try {
            const [solds, rosts, sett] = await Promise.all([
               db.getSoldiers(),
               db.getRosters(),
               db.getSettings()
            ]);
   ...
   const handleConfirmExtraList = async () => {
      ...
      await db.saveExtraDutyHistory(historyEntry);
      for (const s of updatedSoldiers) {
         await db.saveSoldier(s);
      }
   ...
   const handleResetExtraQueue = async () => {
      if (!isAdmin) return;
      ...
      for (let i = 0; i < sortedByRank.length; i++) {
         const s = sortedByRank[i];
         const updated = { ...s, orderExtra: i + 1 };
         await db.saveSoldier(updated);
      }
   ...
   const calculateShift = () => {
      ...
      if (activeRoster) {
         ...
         if (lastRoster) {
            ...
            const projected2x2: Record<number, Set<string>> = { 0: new Set(), 1: new Set() };
         ...
         <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
               <button
                  ...
                  className={`hidden md:flex items-center space-x-2 px-4 py-2 rounded-xl shadow-sm border transition-all ${isTestingSupabase ? 'bg-gray-100 text-gray-400' : 'bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700 text-pm-600 hover:bg-pm-50'}`}
               >
                  <RotateCw className={isTestingSupabase ? 'animate-spin' : ''} size={18} />
                  <span className="text-xs font-bold uppercase">{isTestingSupabase ? 'Testando...' : 'Testar Supabase'}</span>
               </button>
               <button
                  onClick={testXixo}
                  className="flex items-center space-x-2 px-3 py-2 rounded-xl shadow-sm border border-red-100 dark:border-red-900 bg-white dark:bg-slate-800 text-red-600 hover:bg-red-50 transition-all font-bold animate-pulse"
               >
                  <Plus size={18} />
                  <span className="text-[10px] md:text-xs uppercase">Adicionar Xixo</span>
               </button>
               <button
                  onClick={checkXimenes}
                  className="flex items-center space-x-2 px-3 py-2 rounded-xl shadow-sm border border-orange-100 dark:border-orange-900 bg-white dark:bg-slate-800 text-orange-600 hover:bg-orange-50 transition-all font-bold"
               >
                  <Search size={18} />
                  <span className="text-[10px] md:text-xs uppercase">Verificar</span>
               </button>
